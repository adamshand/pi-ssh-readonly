...
The runtime enforcement policy that allows only the registered read-only diagnostic tools plus the **Agent Disconnect Tool** to be active or callable during **SSH Read-only Mode**.
...
**Agent Disconnect Tool**:
The `sshro_disconnect` tool available during **SSH Read-only Mode**, allowing the agent to leave the mode and restore the previous active tool set without human approval.
_Avoid_: remote logout, SSH disconnect, session shutdown
...
- The **SSH Read-only Tool Gate** requires active tools to be exactly the registered read-only diagnostic tools plus the **Agent Disconnect Tool** while **SSH Read-only Mode** is active, warns about unrelated inactive tools, and blocks all other tool calls.
...
- **SSH Read-only Mode** can be entered at startup with `--ssh-ro <target>`, mid-session by the human with `/sshro <target>`, or by the agent with the **Agent Connect Tool**; `/sshro logout` and the **Agent Disconnect Tool** leave SSH Read-only Mode and restore the previous active tool set.
...
- After a successful **Agent Connect Tool** call, the result explicitly says the connection is active, says not to call `sshro_connect` again, terminates the stale pre-connection tool turn, and queues a follow-up message so the next turn starts with the updated `sshro_*` tool set.
- If the **Agent Connect Tool** is called while **SSH Read-only Mode** is already active, it returns a non-error "already connected" message with the active target and remote cwd, says to continue with `sshro_*` tools, terminates the stale turn, and queues the same follow-up.
...
- The **SSHRO Host Whitelist** uses exact string matching against the target argument passed to `sshro_connect`; comma-separated entries are trimmed and empty entries are ignored, but the extension does not canonicalize hosts or parse SSH config, and the **System SSH Client** still resolves aliases, ProxyJump, identities, and other SSH configuration normally when connecting. For example, `binney` and `adam@binney` are distinct targets.
- Human-initiated `/sshro <target>` and `pi --ssh-ro <target>` do not consult the **SSHRO Host Whitelist**; the whitelist only controls auto-approval for agent-initiated **Agent Connect Tool** calls.
- The **Agent Connect Tool** prompt hint includes the exact configured **SSHRO Host Whitelist** target strings, truncated after 20 entries, so the agent knows which targets can auto-connect without prompting; the hint explicitly says automatic approval requires using the target exactly as listed, and if no targets are configured it says the whitelist has no configured targets.
...
- When **SSH Read-only Mode** is active, the extension registers `sshro_read`, `sshro_ls`, `sshro_find`, `sshro_grep`, `sshro_journalctl`, `sshro_systemctl`, `sshro_ps`, `sshro_ss`, `sshro_df`, `sshro_docker_ps`, `sshro_docker_inspect`, `sshro_docker_stats`, `sshro_dig`, and `sshro_disconnect`, sets the active tool list to exactly those tools, and blocks all other agent tool calls defensively; local built-in tools are not active, and stale `sshro_connect` calls are allowed to reach the tool so they can return a clear already-connected recovery message instead of a generic tool-gate error.
...
- If any `sshro_*` read-only diagnostic tool is called while **SSH Read-only Mode** is inactive, it returns a clear not-connected message telling the agent to call `sshro_connect` with an SSH target and to use a whitelist target exactly as listed for automatic approval.
- If the **Agent Disconnect Tool** is called while **SSH Read-only Mode** is inactive, it returns a clear not-connected/nothing-to-disconnect message with the same reconnect guidance.
