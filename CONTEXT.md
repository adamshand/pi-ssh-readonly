...
- `sshro_docker_ps` reads `docker ps --all --no-trunc --format json` by default, omits `--all` only when requested, supports Docker's `--filter name=...`, parses Docker's line-delimited JSON locally, preserves Docker's native JSON field names/shape, preserves Docker command strings and mount summaries, redacts sensitive-looking label values, and returns a pretty JSON array. If JSON output is unavailable or unparseable, it returns a clear error rather than falling back to table output. `limit` must be at least 1, and row truncation appends an explicit note such as `[ssh-ro output truncated to 10 containers]`.
