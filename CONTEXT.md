...
- v1 applies best-effort credential guardrails: direct content reads/searches are blocked for common credential directories/files, shell history files, and password-manager paths. Recursive `sshro_grep` prunes common credential/history/password-manager paths by default. This is not a chroot or adversarial DLP boundary.
...
- `sshro_find` includes hidden files by default, shows blocked credential/history/password-manager entries with a compact `[blocked]` marker, and does not descend into blocked directories during broad traversal.
