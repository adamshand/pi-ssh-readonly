...
- `sshro_docker_ps` reads `docker ps --format json`, parses Docker's line-delimited JSON locally, normalizes rows to lower-case/camel-ish keys, preserves Docker command strings, includes Docker's mount summary, redacts sensitive-looking label values, and returns a pretty JSON array; if JSON output is unavailable or unparseable, it returns a clear error rather than falling back to table output. `limit` must be at least 1, and row truncation appends an explicit note.
- `sshro_docker_inspect` reads Docker inspect JSON and returns curated pretty JSON. Container environment variable values are never shown; if present, they are visibly marked as `[redacted]` so the agent can ask the human operator for help if needed. Docker labels with sensitive-looking keys are redacted. Image `GraphDriver.Data` is omitted. Image/network/volume inspect output otherwise preserves Docker's native shape, so volume mountpoints and network topology/container mappings may be visible.
- `sshro_docker_stats` reads one-shot `docker stats --no-stream` output, parses Docker JSON rows locally, and never starts Docker's streaming stats mode. `limit` must be at least 1, and row truncation appends an explicit note.
